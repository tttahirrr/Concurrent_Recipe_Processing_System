...
def parse_args():
	...
	parser.add_argument('-w', type=int, default=40, help='set wait threshold')
	...
	for line in tscript:
		...
		if prev_start and info.start_time - prev_start.start_time > WAIT_TIME:
			print("ERROR: recipes in queue waiting too long (recipes ready/cooking: " +
                	...
                	str(max_cooks) + ")")
			print("Previous Start Time: " + str(prev_start.start_time) + " Current Start Time: " + str(info.start_time))
			return 3
				...
				if s_check[0] in words[0] and ' '.join(words[1:]) == ' '.join(s_check[1:]):
					...
					if not s[1].recipe.in_progress:
						if cooks + 1 > max_cooks:
							...
							return 2
						if prev_end:
							if info.start_time - prev_end.start_time > AFTER_END_WAIT_TIME:
								print("ERROR: recipes in queue waiting too long (recipes ready/cooking: " +
                              		str(len(ready_set)) + ", active cooks: " + str(cooks) + ", max cooks: " +
                              		str(max_cooks) + ")")
								print("Previous End Time: " + str(prev_end.start_time) + " Current Start Time: " + str(info.start_time))
								return 4
							else:
								prev_end = None
						cooks += 1
...
def compare_output(output, expected_output):
	return eval(output) == expected_output
...
if __name__ == '__main__':
	...
	AFTER_END_WAIT_TIME = 10 * WAIT_TIME
